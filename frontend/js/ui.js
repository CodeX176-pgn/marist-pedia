...
export function showError(element, message) {
    element.innerHTML = `
        <div class="error-state" role="alert">
            ...
            <p>${escapeHtml(message)}</p>
            <button class="button button-primary" data-action="retry" type="button">
                Try Again
            </button>
        </div>
    `;
...
export function renderQuizCard(quiz) {
    ...
    card.innerHTML = `
        <div class="quiz-card-top">
            ...
            <span class="quiz-card-label">Study quiz</span>
        </div>
...
export function renderQuestion(
    ...
    difficulty.textContent = capitalize(question.difficulty || "medium");
    difficulty.dataset.difficulty = question.difficulty || "medium";
    ...
    question.choices.forEach((choice, index) => {
        ...
        option.dataset.answerId = choice.id;
        option.setAttribute("role", "radio");
        option.setAttribute(
            "aria-checked",
            String(selectedAnswer === choice.id),
        );
        option.setAttribute(
            "aria-label",
...
export function renderQuestionNavigator(questions, currentIndex, answers) {
...
export function renderResults(quiz, result) {
    const total = result?.total_questions ?? quiz.questions.length;
    ...
    const review = document.querySelector("#results-review");
    review.innerHTML = "";

    const results = Array.isArray(result?.results) ? result.results : [];
    const resultByQuestion = new Map(
        results.map((item) => [item.question_id, item]),
    );

    quiz.questions.forEach((question, index) => {
        const itemResult = resultByQuestion.get(question.id);
        const item = document.createElement("article");
        const isCorrect = Boolean(itemResult?.is_correct);
        item.className = `result-item ${isCorrect ? "result-correct" : "result-incorrect"}`;

        const selectedChoice = question.choices.find(
            (choice) => choice.id === itemResult?.selected_answer,
        );
        const correctChoice = question.choices.find(
            (choice) => choice.id === itemResult?.correct_answer,
        );

        item.innerHTML = `
            <div class="result-item-header">
                <span class="result-number">${index + 1}</span>
                <span class="result-status">${isCorrect ? "Correct" : "Review"}</span>
            </div>
            <h3>${escapeHtml(question.text)}</h3>
            <div class="result-answer-row">
                <span>Your answer</span>
                <strong>${escapeHtml(selectedChoice?.text || "Not answered")}</strong>
            </div>
            <div class="result-answer-row correct-answer-row">
                <span>Correct answer</span>
                <strong>${escapeHtml(correctChoice?.text || itemResult?.correct_answer || "Unavailable")}</strong>
            </div>
            ${itemResult?.explanation ? `
                <div class="result-explanation">
                    <strong>Why?</strong>
                    <p>${escapeHtml(itemResult.explanation)}</p>
                </div>
            ` : ""}
        `;
...
function getResultMessage(percentage) {
    ...
    if (percentage >= 80) return "Great job. You have a strong grasp of this material.";
    if (percentage >= 60) return "Good progress. Review the missed questions and try again.";
